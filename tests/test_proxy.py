...
class TestProxyManager(unittest.TestCase):
    ...
    def test_save_settings_success(self):
        ...
        try:
            # Create proxy manager and mock the settings path
            with patch.object(ProxyManager, 'get_settings_path', return_value=temp_path):
                proxy = ProxyManager()
    ...
    def test_save_settings_readonly_directory(self):
        """Test save_settings when trying to write to a readonly/nonexistent directory"""
        # Use a path that definitely doesn't exist and can't be created
        readonly_path = "/root/nonexistent/readonly/settings.ini"
        
        with patch.object(ProxyManager, 'get_settings_path', return_value=readonly_path):
            proxy = ProxyManager()
            result = proxy.save_settings()
    ...
    def test_save_settings_exception(self):
        ...
        with patch.object(ProxyManager, 'get_settings_path', return_value='/fake/path'):
            proxy = ProxyManager()
            
            # Patch QSettings only for this specific call
    ...
    def test_save_settings_file_permission_error(self):
        """Test save_settings when file cannot be written (simpler exception test)"""
        # Try to save to a directory that doesn't exist and can't be created
        with patch.object(ProxyManager, 'get_settings_path', return_value='/nonexistent/readonly/settings.ini'):
            proxy = ProxyManager()
            result = proxy.save_settings()
    ...
    def test_save_settings_qsettings_error(self, MockQSettings):
        ...
        # Mock get_settings_path to avoid file system issues
        with patch.object(ProxyManager, 'get_settings_path', return_value='/fake/path'):
            proxy = ProxyManager()
            result = proxy.save_settings()
