...
class SettingsDialog(QDialog):
    ...
    def setup_security_tab(self, tab):
        ...
        # Firewall status
        firewall_status = "Active" if self.app_manager.firewall.is_active() else "Inactive"
        self.status_label = QLabel(firewall_status)
        layout.addRow("Firewall Status:", self.status_label)
        
        # Firewall control
        self.firewall_btn = QPushButton("Enable Firewall" if not self.app_manager.firewall.is_active() else "Disable Firewall")
        self.firewall_btn.clicked.connect(self.toggle_firewall)
        layout.addRow("Firewall Control:", self.firewall_btn)
        
        # Password policy
        self.policy_combo = QComboBox()
        self.policy_combo.addItems(["Basic (8 characters)", "Strong (12 characters)", "Military (16+ characters)"])
    ...
    def setup_privacy_tab(self, tab):
        ...
        self.password_visibility.addItems(["Always masked", "Show when editing", "Always visible"])
    ...
    def view_logs(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error Opening Logs",
                f"Could not open log directory: {str(e)}"
            )
        ...
        if file_path:
            try:
                ...
                QMessageBox.critical(
                    self,
                    "Database Error",
                    f"Failed to move database: {str(e)}"
                )
    ...
    def toggle_firewall(self):
        if self.app_manager.firewall.is_active():
            ...
            if reply == QMessageBox.Yes:
                ...
                QMessageBox.information(self, "Firewall Disabled", "Firewall has been disabled.")
        else:
            # Try to enable firewall
            if self.app_manager.firewall.block_incoming():
                self.status_label.setText("Active")
                self.firewall_btn.setText("Disable Firewall")
                QMessageBox.information(self, "Firewall Enabled", "Firewall has been successfully enabled.")
            else:
                QMessageBox.warning(self, "Firewall Error", 
    ...
    def load_settings(self):
        ...
        log_config = self.app_manager.proxy.settings.get("Logging/config", "Minimal logging")
