...
class ProxyManager:
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.settings["auth_enabled"] = settings.value(
                "auth_enabled", False, type=bool
            )
            self.settings["username"] = settings.value("username", "", type=str)
            self.settings["password"] = settings.value("password", "", type=str)
            self.settings["system_wide"] = settings.value(
                "system_wide", False, type=bool
            )
            settings.endGroup()
    ...
    def clear_system_proxy(self):
        ...
        try:
            if os_type == "Windows":
                ...
                subprocess.run(
                    "networksetup -setwebproxystate Wi-Fi off", shell=True, check=True
                )
            elif os_type == "Linux":
